...
import (
	...
	"github.com/hdfsuite/hdfwallet/internal/cfgutil"
	"github.com/hdfsuite/hdfwallet/internal/legacy/keystore"
	"github.com/hdfsuite/hdfwallet/netparams"
...
const (
	defaultCAFilename       = "hdf.cert"
	defaultConfigFilename   = "hdfwallet.conf"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "hdfwallet.log"
...
var (
	hdfDefaultCAFile  = filepath.Join(hdfutil.AppDataDir("hdf", false), "rpc.cert")
	defaultAppDataDir  = hdfutil.AppDataDir("hdfwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of hdf RPC server to connect to (default localhost:8334, testnet: localhost:18334, simnet: localhost:18556)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with hdf"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	hdfUsername     string                  `long:"hdfusername" description:"Username for hdf authentication"`
	hdfPassword     string                  `long:"hdfpassword" default-mask:"-" description:"Password for hdf authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy RPC and hdf authentication (if hdfusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy RPC and hdf authentication (if hdfpassword is unset)"`
...
//
// The above results in hdfwallet functioning properly without any config
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.UseSPV {
		...
		if cfg.DisableClientTLS {
			...
			// If CAFile is unset, choose either the copy or local hdf cert.
			if !cfg.CAFile.ExplicitlySet() {
				cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

				// If the CA copy does not exist, check if we're connecting to
				// a local hdf and switch to its RPC cert if it exists.
				...
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						hdfCertExists, err := cfgutil.FileExists(
							hdfDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
						...
						if hdfCertExists {
							cfg.CAFile.Value = hdfDefaultCAFile
						}
					}
				}
			}
	...
	// If the hdf username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for hdf and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for hdf and wallet.
	if cfg.hdfUsername == "" {
		cfg.hdfUsername = cfg.Username
	}
	if cfg.hdfPassword == "" {
		cfg.hdfPassword = cfg.Password
	}

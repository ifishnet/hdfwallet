...
func TestHdfAddressSerializer(t *testing.T) {
	...
	addr, err := newHdfAddress(fakeWallet, privKey, nil,
		makeBS(0), true)
	if err != nil {
		t.Error(err.Error())
		return
	...
	if !reflect.DeepEqual(addr, &readAddr) {
		t.Error("Original and read hdfAddress differ.")
	}
}
...
func TestChaining(t *testing.T) {
	...
	for _, test := range tests {
		...
		// the next pubkeys.
		pubkeyUncompressed, err := hdfec.ParsePubKey(nextPubUncompressedFromPub, hdfec.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next uncompressed pubkey: %v", test.name, err)
			return
		}
		pubkeyCompressed, err := hdfec.ParsePubKey(nextPubCompressedFromPub, hdfec.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next compressed pubkey: %v", test.name, err)
			return
		}
		privkeyUncompressed := &hdfec.PrivateKey{
			PublicKey: *pubkeyUncompressed.ToECDSA(),
			D:         new(big.Int).SetBytes(nextPrivUncompressed),
		}
		privkeyCompressed := &hdfec.PrivateKey{
		...
		if !ok {
			t.Errorf("%s: hdfec signature verification failed for next keypair (chained from uncompressed pubkey).",
				test.name)
			return
		}
		...
		if !ok {
			t.Errorf("%s: hdfec signature verification failed for next keypair (chained from compressed pubkey).",
				test.name)
			return
		}
	}
...
func TestWalletPubkeyChaining(t *testing.T) {
	...
	if !ok {
		t.Errorf("hdfec signature verification failed; address's pubkey mismatches the privkey.")
		return
	}
	...
	ok = sig.Verify(hash, pubKey)
	if !ok {
		t.Errorf("hdfec signature verification failed; next address's keypair does not match.")
		return
	}
...
func TestWatchingWalletExport(t *testing.T) {
	...
	pk, _ := hdfec.PrivKeyFromBytes(hdfec.S256(), make([]byte, 32))
	wif, err := hdfutil.NewWIF(pk, tstNetParams, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestImportPrivateKey(t *testing.T) {
	...
	pk, err := hdfec.NewPrivateKey(hdfec.S256())
	if err != nil {
	...
	// import priv key
	wif, err := hdfutil.NewWIF((*hdfec.PrivateKey)(pk), tstNetParams, false)
	if err != nil {
		t.Fatal(err)
	}

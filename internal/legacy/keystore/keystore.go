// Copyright (c) 2013-2016 The hdfsuite developers
...
import (
	...
	"github.com/hdfsuite/hdfwallet/internal/legacy/rename"
)
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := hdfec.PrivKeyFromBytes(hdfec.S256(), privkey)
...
func chainedPrivKey(privkey, pubkey, chaincode []byte) ([]byte, error) {
	...
	case hdfec.PubKeyBytesLenUncompressed, hdfec.PubKeyBytesLenCompressed:
		// Correct length
	default:
...
func chainedPubKey(pubkey, chaincode []byte) ([]byte, error) {
	var compressed bool
	switch n := len(pubkey); n {
	case hdfec.PubKeyBytesLenUncompressed:
		compressed = false
	case hdfec.PubKeyBytesLenCompressed:
		compressed = true
	default:
	...
	oldPk, err := hdfec.ParsePubKey(pubkey, hdfec.S256())
	if err != nil {
		return nil, err
	}
	newX, newY := hdfec.S256().ScalarMult(oldPk.X, oldPk.Y, xorbytes)
	if err != nil {
		return nil, err
	}
	newPk := &hdfec.PublicKey{
		Curve: hdfec.S256(),
		X:     newX,
		Y:     newY,
	}
...
func getAddressKey(addr hdfutil.Address) addressKey {
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]hdfutil.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]hdfutil.Address),
	...
	root, err := newRootHdfAddress(s, rootkey, nil, chaincode,
		createdAt)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]hdfutil.Address)
...
func (s *Store) writeTo(w io.Writer) (n int64, err error) {
	...
	for _, wAddr := range s.addrMap {
		...
		case *scriptAddress:
			...
			copy(e.scriptHash160[:], hdfAddr.AddrHash())
...
func (s *Store) Lock() (err error) {
	...
	for _, addr := range s.addrMap {
		if baddr, ok := addr.(*hdfAddress); ok {
			_ = baddr.lock()
		}
	}
...
func (s *Store) ChangePassphrase(new []byte) error {
	...
	for _, wa := range s.addrMap {
		// Only hdfAddresses curently have private keys.
		a, ok := wa.(*hdfAddress)
		if !ok {
			continue
		}
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (hdfutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedAddress(bs *BlockStamp) (hdfutil.Address, error) {
	addr, err := s.nextChainedHdfAddress(bs)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ChangeAddress(bs *BlockStamp) (hdfutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedHdfAddress(bs *BlockStamp) (*hdfAddress, error) {
...
func (s *Store) LastChainedAddress() hdfutil.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) extendUnlocked(bs *BlockStamp) error {
	...
	newAddr, err := newHdfAddress(s, privkey, nil, bs, true)
	if err != nil {
		return err
	}
...
func (s *Store) extendLocked(bs *BlockStamp) error {
	...
	newaddr, err := newHdfAddressWithoutPrivkey(s, nextPubkey, nil, bs)
	if err != nil {
		return err
	}
	a = newaddr.Address()
...
func (s *Store) createMissingPrivateKeys() error {
	...
	prevAddr, ok := prevWAddr.(*hdfAddress)
	if !ok {
		return errors.New("found non-pubkey chained address")
	}
...
func (s *Store) Address(a hdfutil.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	// Look up address by address hash.
	hdfaddr, ok := s.addrMap[getAddressKey(a)]
	if !ok {
...
func (s *Store) SetSyncStatus(a hdfutil.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *hdfutil.WIF, bs *BlockStamp) (hdfutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := hdfutil.Hash160(wif.SerializePubKey())
	...
	hdfaddr, err := newHdfAddress(s, privKey, nil, bs, wif.CompressPubKey)
	if err != nil {
		return nil, err
	}
	hdfaddr.chainIndex = importedKeyChainIdx

	// Mark as unsynced if import height is below currently-synced
	// height.
	if len(s.recent.hashes) != 0 && bs.Height < s.recent.lastHeight {
		hdfaddr.flags.unsynced = true
	}

	// Encrypt imported address with the derived AES key.
	if err = hdfaddr.encrypt(s.secret); err != nil {
		return nil, err
	}

	addr := hdfaddr.Address()
	...
	s.addrMap[getAddressKey(addr)] = hdfaddr
	s.importedAddrs = append(s.importedAddrs, hdfaddr)
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (hdfutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(hdfutil.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
func (s *Store) ExportWatchingWallet() (*Store, error) {
	...
	ws.keyGenerator = *(kgwc.(*hdfAddress))
	...
	for apkh, addr := range s.addrMap {
		if !addr.Imported() {
			// Must be a hdfAddress if !imported.
			hdfAddr := addr.(*hdfAddress)

			ws.chainIdxMap[hdfAddr.chainIndex] =
				addr.Address()
		}
		apkhCopy := apkh
...
type WalletAddress interface {
	// Address returns a hdfutil.Address for the backing address.
	Address() hdfutil.Address
...
func (s *Store) ActiveAddresses() map[hdfutil.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[hdfutil.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the hdfutil.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]hdfutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (af *addrFlags) ReadFrom(r io.Reader) (int64, error) {
	...
	// hdfwallet shall refuse to open any unencrypted addresses.  This
...
// that hdfwallet fits into an unused space left by Armory's key store file
// format.
...
type PubKeyAddress interface {
	...
	PrivKey() (*hdfec.PrivateKey, error)
	// ExportPrivKey exports the WIF private key.
	ExportPrivKey() (*hdfutil.WIF, error)
}

// newHdfAddress initializes and returns a new address.  privkey must
// be 32 bytes.  iv must be 16 bytes, or nil (in which case it is
// randomly generated).
func newHdfAddress(wallet *Store, privkey, iv []byte, bs *BlockStamp, compressed bool) (addr *hdfAddress, err error) {
	if len(privkey) != 32 {
		return nil, errors.New("private key is not 32 bytes")
	}

	addr, err = newHdfAddressWithoutPrivkey(wallet,
...
// newHdfAddressWithoutPrivkey initializes and returns a new address with an
...
// randomly generated).
func newHdfAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *hdfAddress, err error) {
	var compressed bool
	switch n := len(pubkey); n {
	case hdfec.PubKeyBytesLenCompressed:
		compressed = true
	...
	pk, err := hdfec.ParsePubKey(pubkey, hdfec.S256())
	if err != nil {
		return nil, err
	}

	address, err := hdfutil.NewAddressPubKeyHash(hdfutil.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
// newRootHdfAddress generates a new address, also setting the
// chaincode and chain index to represent this address as a root
// address.
func newRootHdfAddress(s *Store, privKey, iv, chaincode []byte,
	bs *BlockStamp) (addr *hdfAddress, err error) {

	if len(chaincode) != 32 {
		return nil, errors.New("chaincode is not 32 bytes")
	}

	// Create new hdfAddress with provided inputs.  This will
	// always use a compressed pubkey.
	addr, err = newHdfAddress(s, privKey, iv, bs, true)
	if err != nil {
		return nil, err
	}
...
// unlocked hdfAddress.
func (a *hdfAddress) verifyKeypairs() error {
...
func (a *hdfAddress) ReadFrom(r io.Reader) (n int64, err error) {
	var read int64
	...
	addr, err := hdfutil.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *hdfAddress) WriteTo(w io.Writer) (n int64, err error) {
	var written int64
...
func (a *hdfAddress) encrypt(key []byte) error {
...
func (a *hdfAddress) unlock(key []byte) (privKeyCT []byte, err error) {
	if !a.flags.encrypted {
	...
	x, y := hdfec.S256().ScalarBaseMult(privkey)
...
func (a *hdfAddress) changeEncryptionKey(oldkey, newkey []byte) error {
...
func (a *hdfAddress) SyncStatus() SyncStatus {
	switch {
...
// PubKeyAddress.
func (a *hdfAddress) PubKey() *hdfec.PublicKey {
	return a.pubKey
}

func (a *hdfAddress) pubKeyBytes() []byte {
	if a.Compressed() {
...
func (a *hdfAddress) ExportPubKey() string {
...
func (a *hdfAddress) PrivKey() (*hdfec.PrivateKey, error) {
...
func (a *hdfAddress) ExportPrivKey() (*hdfutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}
	...
	// elsewhere.
	wif, err := hdfutil.NewWIF((*hdfec.PrivateKey)(pk), a.store.netParams(),
		a.Compressed())
	if err != nil {
		panic(err)
	}
...
// normal key store.
func (a *hdfAddress) watchingCopy(s *Store) walletAddress {
	return &hdfAddress{
		store:   s,
		address: a.address,
		flags: addrFlags{
...
func (a *hdfAddress) setSyncStatus(s SyncStatus) {
	switch e := s.(type) {
	case Unsynced:
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	scriptHash := hdfutil.Hash160(script)

	address, err := hdfutil.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
func (sa *scriptAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	address, err := hdfutil.NewAddressScriptHashFromHash(scriptHash[:],
		sa.store.netParams())
	if err != nil {
		return n, err
	}
...
// address returns a hdfutil.AddressScriptHash for a hdfAddress.
func (sa *scriptAddress) Address() hdfutil.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []hdfutil.Address {
	return sa.addresses
}

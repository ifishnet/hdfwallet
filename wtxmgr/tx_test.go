...
var (
	...
	TstRecvTx, _                    = hdfutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = hdfutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *hdfutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := hdfutil.NewTxFromBytes(TstRecvSerializedTx)
	// Switch txout amount to 1 HDF.  Transaction store doesn't
	...
	TstDoubleSpendTx := hdfutil.NewTx(newDupMsgTx)
	...
	TstSpendingTx := hdfutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: hdfutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: hdfutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: hdfutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: hdfutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: hdfutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: hdfutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Hash(),
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := hdfutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
...
func TestCoinbases(t *testing.T) {
	...
	// Balance should be 0 if the coinbase is immature, 50 HDF at and beyond
	// maturity.
	//
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance hdfutil.Amount,
	...
	checkBalance(hdfutil.Amount(initialBalance), false)
	checkBalance(hdfutil.Amount(initialBalance), true)
	...
	checkBalance(hdfutil.Amount(changeAmount), true)
...
func assertBalance(t *testing.T, s *Store, ns walletdb.ReadWriteBucket,
	confirmed bool, blockHeight int32, exp hdfutil.Amount) {
...
func TestOutputLocks(t *testing.T) {
	...
	coinbase := newCoinBase(
		hdfutil.SatoshiPerBitcoin, hdfutil.SatoshiPerBitcoin*2,
	)
	coinbaseHash := coinbase.TxHash()

	// One of the spends will be unconfirmed.
	const unconfirmedBalance = hdfutil.SatoshiPerBitcoin / 2
	...
	const confirmedBalance = hdfutil.SatoshiPerBitcoin

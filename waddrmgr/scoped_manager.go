...
func (s *ScopedKeyManager) importedAddressRowToManaged(row *dbImportedAddressRow) (ManagedAddress, error) {
	...
	pubKey, err := hdfec.ParsePubKey(pubBytes, hdfec.S256())
	if err != nil {
	...
	compressed := len(pubBytes) == hdfec.PubKeyBytesLenCompressed
...
func (s *ScopedKeyManager) Address(ns walletdb.ReadBucket,
	...
	if pka, ok := address.(*hdfutil.AddressPubKey); ok {
...
func (s *ScopedKeyManager) AddrAccount(ns walletdb.ReadBucket,
	address hdfutil.Address) (uint32, error) {
...
func (s *ScopedKeyManager) ImportPrivateKey(ns walletdb.ReadWriteBucket,
	wif *hdfutil.WIF, bs *BlockStamp) (ManagedPubKeyAddress, error) {
	...
	pubKeyHash := hdfutil.Hash160(serializedPubKey)
	...
	if !s.rootManager.WatchOnly() {
		...
		pubKey := (*hdfec.PublicKey)(&wif.PrivKey.PublicKey)

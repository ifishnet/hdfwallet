// Copyright (c) 2014 The hdfsuite developers
...
// Address returns the hdfutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() hdfutil.Address {
	return a.address
}
...
func (a *managedAddress) PubKey() *hdfec.PublicKey {
	return a.pubKey
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) PrivKey() (*hdfec.PrivateKey, error) {
	...
	privKey, _ := hdfec.PrivKeyFromBytes(hdfec.S256(), privKeyCopy)
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*hdfutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return hdfutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	derivationPath DerivationPath, pubKey *hdfec.PublicKey, compressed bool,
	...
	if compressed {
		...
		pubKeyHash = hdfutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address hdfutil.Address
	var err error
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := hdfutil.NewAddressWitnessPubKeyHash(
		...
		address, err = hdfutil.NewAddressScriptHash(
	...
	case WitnessPubKey:
		address, err = hdfutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
func newManagedAddress(s *ScopedKeyManager, derivationPath DerivationPath,
	privKey *hdfec.PrivateKey, compressed bool,
	addrType AddressType) (*managedAddress, error) {
	...
	ecPubKey := (*hdfec.PublicKey)(&privKey.PublicKey)
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *hdfutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() hdfutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := hdfutil.NewAddressScriptHashFromHash(

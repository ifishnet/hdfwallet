...
// NeutrinoClient is an implementation of the hdfwalet chain.Interface interface.
...
func (s *NeutrinoClient) GetBlock(hash *chainhash.Hash) (*wire.MsgBlock, error) {
	...
	//  Should the block cache be INSIDE neutrino instead of in hdfwallet?
...
func (s *NeutrinoClient) Rescan(startHash *chainhash.Hash, addrs []hdfutil.Address,
...
func (s *NeutrinoClient) NotifyBlocks() error {
	...
	if !s.scanning {
		s.clientMtx.Unlock()
		return s.NotifyReceived([]hdfutil.Address{})
	}
	s.clientMtx.Unlock()
	return nil
}
...
func (s *NeutrinoClient) NotifyReceived(addrs []hdfutil.Address) error {
	s.clientMtx.Lock()
...
func (s *NeutrinoClient) onFilteredBlockConnected(height int32,
	header *wire.BlockHeader, relevantTxs []*hdfutil.Tx) {

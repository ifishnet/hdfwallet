...
func (c *BitcoindClient) GetBlockVerbose(
	hash *chainhash.Hash) (*hdfjson.GetBlockVerboseResult, error) {
...
func (c *BitcoindClient) GetBlockHeaderVerbose(
	hash *chainhash.Hash) (*hdfjson.GetBlockHeaderVerboseResult, error) {
...
func (c *BitcoindClient) GetRawTransactionVerbose(
	hash *chainhash.Hash) (*hdfjson.TxRawResult, error) {
...
func (c *BitcoindClient) GetTxOut(txHash *chainhash.Hash, index uint32,
	mempool bool) (*hdfjson.GetTxOutResult, error) {
...
//
// NOTE: This is part of the chain.Interface interface.
func (c *BitcoindClient) NotifyReceived(addrs []hdfutil.Address) error {
	c.NotifyBlocks()
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]hdfutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *BitcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []hdfutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]hdfutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
// matched as []hdfjson.BlockDetails.
func (c *BitcoindClient) RescanBlocks(
	blockHashes []chainhash.Hash) ([]hdfjson.RescannedBlock, error) {

	rescannedBlocks := make([]hdfjson.RescannedBlock, 0, len(blockHashes))
	for _, hash := range blockHashes {
		...
		if len(relevantTxs) > 0 {
			rescannedBlock := hdfjson.RescannedBlock{
				Hash: hash.String(),
			}
...
func (c *BitcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []hdfutil.Address, outPoints map[wire.OutPoint]hdfutil.Address) error {
...
func (c *BitcoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]hdfutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
...
func (c *BitcoindClient) rescan(start chainhash.Hash) error {
	...
	for i := previousHeader.Height + 1; i <= bestBlock.Height; i++ {
			...
			if headers.Back() != nil {
				...
				if headers.Back() != nil {
					previousHeader = headers.Back().
						Value.(*hdfjson.GetBlockHeaderVerboseResult)
		...
		previousHeader = &hdfjson.GetBlockHeaderVerboseResult{

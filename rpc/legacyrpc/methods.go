...
import (
	...
	"github.com/ifishnet/hdf/rpcclient"
	...
	"github.com/hdfsuite/hdfwallet/wallet/txrules"
...
// request into a marshalable response.  If the error is a *hdfjson.RPCError
...
// catch-all error code, hdfjson.ErrRPCWallet.
...
// context.
type lazyHandler func() (interface{}, *hdfjson.RPCError)
...
func lazyApplyHandler(request *hdfjson.Request, w *wallet.Wallet, chainClient chain.Interface) lazyHandler {
	handlerData, ok := rpcHandlers[request.Method]
	if ok && handlerData.handlerWithChain != nil && w != nil && chainClient != nil {
		return func() (interface{}, *hdfjson.RPCError) {
			cmd, err := hdfjson.UnmarshalCmd(request)
			if err != nil {
				return nil, hdfjson.ErrRPCInvalidRequest
			}
...
func makeResponse(id, result interface{}, err error) hdfjson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return hdfjson.Response{
			ID:    idPtr,
			Error: jsonError(err),
		}
	}
...
func jsonError(err error) *hdfjson.RPCError {
	if err == nil {
		return nil
	}
	...
	case waddrmgr.ManagerError:
		...
		case waddrmgr.ErrWrongPassphrase:
			code = hdfjson.ErrRPCWalletPassphraseIncorrect
		}
	}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*hdfutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := hdfutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.AddMultisigAddressCmd)
	...
	secp256k1Addrs := make([]hdfutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.CreateMultisigCmd)
	...
	address, err := hdfutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
		// above is a valid script, shouldn't happen.
		return nil, err
	}

	return hdfjson.CreateMultiSigResult{
...
func dumpPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.DumpPrivKeyCmd)
...
func getAddressesByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.GetAddressesByAccountCmd)
...
func getBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.GetBalanceCmd)
...
// information about the current state of hdfwallet.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	// Call down to hdf for all of the information in this command known
	// by them.
...
func decodeAddress(s string, params *chaincfg.Params) (hdfutil.Address, error) {
	addr, err := hdfutil.DecodeAddress(s, params)
	if err != nil {
...
func getAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.GetAccountCmd)
...
// appear in the blockchain, or any tx that has arrived in the hdf mempool).
// If the most recently-requested address has been used, a new address (the
// next chained address in the keypool) is used.  This can fail if the keypool
// runs out (and will return hdfjson.ErrRPCWalletKeypoolRanOut if that happens).
func getAccountAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.GetAccountAddressCmd)
...
func getUnconfirmedBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.GetUnconfirmedBalanceCmd)
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := hdfutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &hdfjson.RPCError{
			Code:    hdfjson.ErrRPCInvalidAddressOrKey,
...
func createNewAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.CreateNewAccountCmd)
...
func renameAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.RenameAccountCmd)
...
func getNewAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.GetNewAddressCmd)
...
func getRawChangeAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.GetRawChangeAddressCmd)
...
func getReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.GetReceivedByAccountCmd)
	...
	return results[acctIndex].TotalReceived.ToHDF(), nil
}
...
func getReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.GetReceivedByAddressCmd)
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	ret := hdfjson.GetTransactionResult{
		TxID:            cmd.Txid,
	...
	var (
		debitTotal  hdfutil.Amount
		creditTotal hdfutil.Amount // Excludes change
		fee         hdfutil.Amount
		feeF64      float64
	)
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var outputTotal hdfutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += hdfutil.Amount(output.Value)
		}
	...
	if len(details.Debits) == 0 {
		...
		ret.Details = make([]hdfjson.GetTransactionDetailsResult, 0, len(details.Credits))
	} else {
		ret.Details = make([]hdfjson.GetTransactionDetailsResult, 1, len(details.Credits)+1)

		ret.Details[0] = hdfjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Account
			//   Address
			...
			Category: "send",
			Amount:   (-debitTotal).ToHDF(), // negative since it is a send
			Fee:      &feeF64,
		}
		ret.Fee = feeF64
	}
	...
	for _, cred := range details.Credits {
		...
		ret.Details = append(ret.Details, hdfjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Fee
			Account:  accountName,
			Address:  address,
			Category: credCat,
			Amount:   cred.Amount.ToHDF(),
			Vout:     cred.Index,
		})
	}

	ret.Amount = creditTotal.ToHDF()
	return ret, nil
}
...
func help(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*hdfjson.HelpCmd)

	// hdf returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to hdf.  Therefore, create a
	...
	// texts in a non-internal hdf package.
...
func listAccounts(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.ListAccountsCmd)
	...
	for _, result := range results {
		accountBalances[result.AccountName] = result.AccountBalance.ToHDF()
	}
...
func listReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.ListReceivedByAccountCmd)
	...
	jsonResults := make([]hdfjson.ListReceivedByAccountResult, 0, len(results))
	for _, result := range results {
		jsonResults = append(jsonResults, hdfjson.ListReceivedByAccountResult{
			Account:       result.AccountName,
			Amount:        result.TotalReceived.ToHDF(),
...
func listReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.ListReceivedByAddressCmd)
	...
	ret := make([]hdfjson.ListReceivedByAddressResult, numAddresses, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = hdfjson.ListReceivedByAddressResult{
			Address:       address,
			Amount:        addrData.amount.ToHDF(),
...
func listSinceBlock(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*hdfjson.ListSinceBlockCmd)
	...
	res := hdfjson.ListSinceBlockResult{
...
func listTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.ListTransactionsCmd)
...
func listAddressTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.ListAddressTransactionsCmd)
...
func listAllTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.ListAllTransactionsCmd)
...
func listUnspent(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.ListUnspentCmd)
...
func lockUnspent(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.LockUnspentCmd)
...
// and amounts.
func makeOutputs(pairs map[string]hdfutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	outputs := make([]*wire.TxOut, 0, len(pairs))
	for addrStr, amt := range pairs {
		addr, err := hdfutil.DecodeAddress(addrStr, chainParams)
		if err != nil {
...
// All errors are returned in hdfjson.RPCError format
func sendPairs(w *wallet.Wallet, amounts map[string]hdfutil.Amount,
	account uint32, minconf int32, feeSatPerKb hdfutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := hdfutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]hdfutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]hdfutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := hdfutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func sendToAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.SendToAddressCmd)
...
func signMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.SignMessageCmd)
	...
	sigbytes, err := hdfec.SignCompact(hdfec.S256(), privKey,
		messageHash, true)
	if err != nil {
		return nil, err
	}
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*hdfjson.SignRawTransactionCmd)
	...
	// TODO: really we probably should look these up with hdf anyway to
	// make sure that they match the blockchain if present.
	inputs := make(map[wire.OutPoint][]byte)
	scripts := make(map[string][]byte)
	var cmdInputs []hdfjson.RawTxInput
	if cmd.Inputs != nil {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := hdfutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	// querying hdf with getrawtransaction. We queue up a bunch of async
	...
	var keys map[string]*hdfutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*hdfutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := hdfutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := hdfutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	signErrors := make([]hdfjson.SignRawTransactionError, 0, len(signErrs))
	for _, e := range signErrs {
		input := tx.TxIn[e.InputIndex]
		signErrors = append(signErrors, hdfjson.SignRawTransactionError{
	...
	return hdfjson.SignRawTransactionResult{
...
func validateAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.ValidateAddressCmd)

	result := hdfjson.ValidateAddressWalletResult{}
	addr, err := decodeAddress(cmd.Address, w.ChainParams())
	if err != nil {
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.VerifyMessageCmd)
	...
	pk, wasCompressed, err := hdfec.RecoverCompact(hdfec.S256(), sig,
		expectedMessageHash)
	if err != nil {
		return nil, err
	}
	...
	case *hdfutil.AddressPubKeyHash: // ok
		return bytes.Equal(hdfutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *hdfutil.AddressPubKey: // ok
...
func walletPassphrase(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.WalletPassphraseCmd)
...
func walletPassphraseChange(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*hdfjson.WalletPassphraseChangeCmd)

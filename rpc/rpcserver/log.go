...
func UseLogger(l hdflog.Logger) {
	grpclog.SetLogger(logger{l})
}

// logger uses a hdflog.Logger to implement the grpclog.Logger interface.
type logger struct {
	hdflog.Logger
}

// stripGrpcPrefix removes the package prefix for all logs made to the grpc
// logger, since these are already included as the hdflog subsystem name.

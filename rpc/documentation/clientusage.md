...
import (
	...
	pb "github.com/hdfsuite/hdfwallet/rpc/walletrpc"
...
var certificateFile = filepath.Join(hdfutil.AppDataDir("hdfwallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", hdfutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/hdfwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/hdfwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by hdfwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.hdfwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\hdfwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by hdfwallet).
            Environment.SetEnvironmentVariable("GRPC_SSL_CIPHER_SUITES", "HIGH+ECDSA");

            var walletAppData = Portability.LocalAppData(Environment.OSVersion.Platform, "Hdfwallet");
...
var certPath = path.join(process.env.HOME, '.hdfwallet', 'rpc.cert');
if (os.platform == 'win32') {
    certPath = path.join(process.env.LOCALAPPDATA, 'Hdfwallet', 'rpc.cert');
...
```bash
$ protoc -I /path/to/hdfsuite/hdfwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    cert_file_path = os.path.join(os.environ['HOME'], '.hdfwallet', 'rpc.cert')
    if platform.system() == 'Windows':
        cert_file_path = os.path.join(os.environ['LOCALAPPDATA'], "Hdfwallet", "rpc.cert")

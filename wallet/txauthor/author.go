...
import (
	...
	"github.com/hdfsuite/hdfwallet/wallet/txsizes"
)

// SumOutputValues sums up the list of TxOuts and returns an Amount.
func SumOutputValues(outputs []*wire.TxOut) (totalOutput hdfutil.Amount) {
	for _, txOut := range outputs {
		totalOutput += hdfutil.Amount(txOut.Value)
	}
	return totalOutput
}
...
// InputSourceError.
type InputSource func(target hdfutil.Amount) (total hdfutil.Amount, inputs []*wire.TxIn,
	inputValues []hdfutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []hdfutil.Amount
	TotalInput      hdfutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, feeRatePerKb hdfutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []hdfutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := hdfutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}

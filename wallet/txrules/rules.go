// Copyright (c) 2016 The hdfsuite developers
...
const DefaultRelayFeePerKb hdfutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb hdfutil.Amount) hdfutil.Amount {
	...
	relayFee := hdfutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount hdfutil.Amount, scriptSize int, relayFeePerKb hdfutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb hdfutil.Amount) bool {
	...
	return IsDustAmount(hdfutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb hdfutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > hdfutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb hdfutil.Amount, txSerializeSize int) hdfutil.Amount {
	fee := relayFeePerKb * hdfutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > hdfutil.MaxSatoshi {
		fee = hdfutil.MaxSatoshi
	}

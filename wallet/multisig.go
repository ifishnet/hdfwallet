...
func (w *Wallet) MakeMultiSigScript(addrs []hdfutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*hdfutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*hdfutil.AddressScriptHash, error) {
	var p2shAddr *hdfutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = hdfutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*hdfutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
